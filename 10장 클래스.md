...
> ### 변경으로부터 격리
요구사항은 변하기 마련이고 이에따라 코드도 변하기 마련이다.
...
상세한 구현에 의존하는클래스는 구현이 바뀌면 위험에 빠지기 때문에<br/>
우리는 인터페이스와 추상클래스를 사용해 구현에 미치는 영향을 구분한다.
...
이와같은 방법은 방법으로 코드가 변하더라고 구현에 미치는 영향을 최소화 할 수 있다.
